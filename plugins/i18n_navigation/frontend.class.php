...
class I18nNavigationFrontend
{
            ...
            foreach ($pages[$slug]['children'] as $childslug) {
                if ($childslug != $slugToIgnore && (!$menuOnly || $pages[$childslug]['menuStatus'] == 'Y')) {
                    if (!$lang || isset($pages[$childslug]['title_'.$lang])) {
                        ...
                        if ($lang && isset($pages[$childslug]['menu_'.$lang])) {
                            $menu = $pages[$childslug]['menu_'.$lang];
                        }
                        $structure[] = array(
                        'level' => $level,
                        'url' => $childslug,
    ...
    public static function getMenu($slug, $minlevel = 0, $maxlevel = 0, $show = I18N_SHOW_NORMAL)
    {
      ...
      // find last page in breadcrumbs that is displayed in the menu
        for ($icu = 0; $icu+1 < count($breadcrumbs) && $pages[$breadcrumbs[$icu+1]]['menuStatus'] == 'Y';
        $icu++) {
        }
      ...
      // menus to display
        if ($minlevel < 0 || $maxlevel < $minlevel || $minlevel >= count($breadcrumbs)) {
            ...
            return self::getMenuImpl($breadcrumbs, $currenturl, $breadcrumbs[$minlevel], $maxlevel-$minlevel+1, 0, $show);
        }
    }

    private static function getMenuImpl($breadcrumbs, $currenturl, $url, $levels, $lcount, $show = I18N_SHOW_NORMAL)
    {
        global $language; // only set if I18N base plugin is available
        $pages = self::getPages();
        if (!@$pages[$url] || $levels <= 0) {
            return null;
        }
        ...
        if (isset($pages[$url]['children'])) {
            foreach ($pages[$url]['children'] as $childurl) {
                ...
                if ($showIt) {
                    ...
                    $children = $showChildren ? self::getMenuImpl($breadcrumbs, $currenturl, $childurl, $levels-1, $lcount+1, $show) : null;
                    $menu[] = array(
                    'url' => $childurl,
    ...
    private static function hasChildren($url, $show = I18N_SHOW_NORMAL)
    {
        global $language; // only set if I18N base plugin is available
        $pages = self::getPages();
        if (!@$pages[$url] || !isset($pages[$url]['children'])) {
            return false;
        }
        $deflang = function_exists('return_i18n_default_language') ? return_i18n_default_language() : null;
        foreach ($pages[$url]['children'] as $childurl) {
    ...
    private static function getProperty($url, $key, $deflang)
    {
        ...
        if ($deflang !== null) {
            ...
            foreach ($languages as $language) {
                ...
                if (isset($pages[$url][$fullkey])) {
                    return $pages[$url][$fullkey];
                }
            }
        } else {
            if (isset($pages[$url][$key])) {
    ...
    public function __get($name)
    {
        switch ($name) {
            ...
            case 'link':
                ...
                } elseif (function_exists('find_i18n_url')) {
