...
function i18n_gallery_from_request($languages)
{
    ...
    if (count($languages) > 0) {
        foreach ($languages as $language) {
            if (@$_POST['post-title_'.$language]) {
                $gallery['title_'.$language] = $_POST['post-title_'.$language];
            }
        }
    }
    ...
    foreach ($_POST as $key => $value) {
        if (substr($key, 0, strlen($type)+1) == $type.'-') {
            $gallery[substr($key, strlen($type)+1)] = $value;
        }
    ...
    for ($i=0; isset($_POST['post-item_'.$i.'_filename']); $i++) {
      ...
      'height' => $height
        ...
        if (count($languages) > 0) {
            foreach ($languages as $language) {
                if (@$_POST['post-item_'.$i.'_title_'.$language]) {
                    $gal['title_'.$language] = $_POST['post-item_'.$i.'_title_'.$language];
                }
                if (@$_POST['post-item_'.$i.'_description_'.$language]) {
                    $gal['description_'.$language] = $_POST['post-item_'.$i.'_description_'.$language];
                }
            }
        }
        $gallery['items'][] = $gal;
...
if (!I18nGallery::checkPrerequisites()) {
...
} elseif (isset($_GET['undo']) && !isset($_POST['save'])) {
    ...
    } elseif (!@$_POST['post-item_0_filename']) {
        $msg = i18n_r('i18n_gallery/ERR_NO_IMAGES');
    } elseif (@$_POST['post-name'] != @$_GET['name'] && file_exists(GSDATAPATH.I18N_GALLERY_DIR.@$_POST['post-name'].'.xml')) {
...
<?php if (count($languages) > 0) {
    foreach ($languages as $language) {
        ?>
        <input type="text" class="text title lang lang_<?php echo $language;
        ?>" name="post-title_<?php echo $language;
        ?>" value="<?php echo htmlspecialchars(@$gallery['title_'.$language]);
        ?>" style="display:none"/>
<?php
    }
} ?>
...
<?php if (count($plugins) > 0) {
    foreach ($plugins as $plugin) {
        ?>
              <option value="<?php echo $plugin['type'];
        ?>" <?php echo $plugin['type'] == @$gallery['type'] ? 'selected="selected"' : '';
...
<?php if (count($plugins) > 0) {
    foreach ($plugins as $plugin) {
        ?>
          <p class="type type_<?php echo $plugin['type'];
        ?>" style="display:<?php echo $plugin['type'] == @$gallery['type'] ? 'block' : 'none';
        ?>">
...
<?php if (count($plugins) > 0) {
    foreach ($plugins as $plugin) {
        ?>
        <div class="rightopt">
    			<div class="type type_<?php echo $plugin['type'];
        ?>" style="display:<?php echo $plugin['type'] == @$gallery['type'] ? 'block' : 'none';
        ?>">
...
<?php
  ...
  if (count(@$gallery['items']) > 0) {
      foreach ($gallery['items'] as $item) {
          ...
          ?>
          <tr>
            <td><a href="#" class="setimage"><img src="<?php echo $tlink;
          ?>"/></a></td>
            <td>
              <input type="hidden" name="post-item_<?php echo $i;
          ?>_filename" value="<?php echo htmlspecialchars($item['filename']);
          ?>"/>
              <span class="imagefile" style="float:left;width:<?php echo 400-$w;
          ?>px"><?php echo htmlspecialchars($item['filename']);
          ?></span>
              <span class="imagesize" style="float:left;width:120px;"><?php echo $item['width'] . " x " . $item['height'];
          ?></span>
              <span class="imagebytes" style="float:left;width:80px;text-align:right"><?php echo $s;
          ?></span>
              <input type="text" class="text lang lang_" name="post-item_<?php echo $i;
          ?>_title" value="<?php echo htmlspecialchars(@$item['title']);
          ?>"
                title="<?php echo htmlspecialchars(i18n_r('i18n_gallery/TITLE'));
          ?>" style="clear:both;float:left;width:<?php echo 383-$w;
          ?>px;margin-right:5px;"/>
<?php if (count($languages) > 0) {
    foreach ($languages as $language) {
        ?>
              <input type="text" class="text lang lang_<?php echo $language;
        ?>" name="post-item_<?php echo $i;
        ?>_title_<?php echo $language;
        ?>" value="<?php echo htmlspecialchars(@$item['title_'.$language]);
        ?>"
                title="<?php echo htmlspecialchars(i18n_r('i18n_gallery/TITLE'));
        ?>" style="clear:both;float:left;width:<?php echo 383-$w;
        ?>px;margin-right:5px;display:none;"/>
<?php
    }
}
          ?>
              <input type="text" class="text" name="post-item_<?php echo $i;
          ?>_tags" value="<?php echo htmlspecialchars(@$item['tags']);
          ?>"
                title="<?php echo htmlspecialchars(i18n_r('i18n_gallery/TAGS'));
          ?>" style="float:left;width:188px"/>
              <textarea class="text lang lang_" name="post-item_<?php echo $i;
          ?>_description" title="<?php echo htmlspecialchars(i18n_r('i18n_gallery/DESCRIPTION'));
          ?>"
                style="clear:both;float:left;width:<?php echo 588-$w;
          ?>px;height:14px;margin-top:2px;"><?php echo htmlspecialchars(@$item['description']);
          ?></textarea>
<?php if (count($languages) > 0) {
    foreach ($languages as $language) {
        ?>
              <textarea class="text lang lang_<?php echo $language;
        ?>" name="post-item_<?php echo $i;
        ?>_description_<?php echo $language;
        ?>" title="<?php echo htmlspecialchars(i18n_r('i18n_gallery/DESCRIPTION'));
        ?>"
                style="clear:both;float:left;width:<?php echo 588-$w;
        ?>px;height:14px;margin-top:2px;display:none;"><?php echo htmlspecialchars(@$item['description_'.$language]);
        ?></textarea>
<?php
    }
}
          ?>
	          ...
	          <td class="delete" >
              <a href="#" title="<?php i18n('i18n_gallery/DELETE_ITEM');
          ?>">X</a>
              <input type="hidden" name="post-item_<?php echo $i;
          ?>_latitude" value="<?php echo htmlspecialchars($item['latitude']);
          ?>"/>
              <input type="hidden" name="post-item_<?php echo $i;
          ?>_longitude" value="<?php echo htmlspecialchars($item['longitude']);
          ?>"/>
              <span class="geo <?php echo $item['latitude'] && $item['longitude'] ? 'geo-yes' : '';
          ?>"> </span>
            </td>
          </tr>
<?php
...
<?php if (@$name) {
    ?>
      &nbsp;/&nbsp;
      <a class="cancel" href="load.php?id=i18n_gallery&amp;overview&amp;name=<?php echo $name;
    ?>&amp;delete"><?php i18n('i18n_gallery/DELETE');
    ?></a>
<?php
} ?>
    </form>
...
<?php if (count($languages) > 0) {
    foreach ($languages as $language) {
        ?>
        html += '<input type="text" class="text lang lang_<?php echo $language;
        ?>" name="post-item_'+i+'_title<?php echo $language ? '_'.$language : '';
        ?>" value="" style="clear:both;float:left;width:<?php echo 383-$w;
        ?>px;margin-right:5px;display:none;"/>';
<?php
    }
} ?>
...
<?php if (count($languages) > 0) {
    foreach ($languages as $language) {
        ?>
        html += '<textarea class="text lang lang_<?php echo $language;
        ?>" name="post-item_'+i+'_description<?php echo $language ? '_'.$language : '';
        ?>" style="clear:both;float:left;width:<?php echo 588-$w;
        ?>px;margin-top:2px;height:14px;display:none;"></textarea>';
<?php
    }
} ?>
        html += '</td>\n';
...
<?php if (isset($msg)) {
    ?>
        $('div.bodycontent').before('<div class="<?php echo $success ? 'updated' : 'error';
    ?>" style="display:block;">'+<?php echo json_encode($msg);
    ?>+'</div>');
